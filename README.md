...
- Real Gemini transcription and structured summary generation through the Python backend
- Gemini summaries in Hindi, Gujarati, and English, with short, bullet, and detailed modes
...
audio or generating a summary. Keep Gemini credentials on the backend. Supported audio imports are AAC,
...
for retry. Provider errors are shown without saving a result or consuming summary
usage. Both imported audio and text submitted for summarization are sent to Gemini;
free-tier Gemini quotas and data handling apply. Mock summaries are available only
when mock mode is explicitly selected.
...
1. Evaluate Hindi, Gujarati, and mixed-language recordings with Gemini
2. Add real audio recording from the app
3. Replace Pro preview with a real purchase flow
